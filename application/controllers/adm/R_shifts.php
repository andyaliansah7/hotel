...
class R_shifts extends BaseController
{
	/**
	...
	public function index()
 	{	
        $data['content_title'] = 'Laporan - Room Count Sheet';
		
		$this->twiggy_display('adm/r_shifts/index', $data);
	}
	...
	{	
		...
		$where_header_3 = array('transaction_type' => 'C', 'shift_id' => '3', 'date_in' => $date_1, 'date_out >' => $date_2, 'd.room_type_group !=' => 'Function Room');

		$order_header   = 'id ASC';
		$get_data_1     = $this->Checkin_model->get_data($where_header_1)->result();
		$get_data_2     = $this->Checkin_model->get_data($where_header_2)->result();
		$get_data_3     = $this->Checkin_model->get_data($where_header_3)->result();
		...
		{	
			...
			{	
				...
				// ROOM RATE
				$special_where = array('a.room_type_id' => $get_row->room_type_id, 'special_rate_date' => $date_1);
				$special_price = $this->Special_rates_model->get_data($special_where)->row();
			
				if($get_row->get_price == 1){
					$room_rate          = $get_row->total_price;
				}else{
					if($special_price){
						$room_rate      = $special_price->special_rate_price;
					}else{
		...
		{	
			...
			{	
				...
				if($get_data_check){
					...
					// $room_rate      = $get_data_check->weekday_price;
					// if($date_now_check == 'Sat'){
					// 	$room_rate  = $get_data_check->weekend_price;
					// }

					// ROOM RATE
					$special_where = array('a.room_type_id' => $get_data_check->room_type_id, 'special_rate_date' => $date_1);
					$special_price = $this->Special_rates_model->get_data($special_where)->row();
		...
		$total_extra_bed_all          = ($total_extra_bed_1 + $total_extra_bed_2 + $total_extra_bed_3);
		$total_pax_all                = ($total_pax_1 + $total_pax_2 + $total_pax_3);
