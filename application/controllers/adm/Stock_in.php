...
class Stock_in extends BaseController
{
	...
	public function index()
 	{	
		$data['content_title'] = 'Stok Masuk';
		$this->twiggy_display('adm/stock_in/index', $data);
	}
	...
	{	
		$order    = "number desc";
		$data     = [];
		$get_data = $this->Stock_in_model->stockin_header('', $order)->result();
 	...
 	public function edit($id = 'new')
	{
		...
		$prefix           = "STI";
		$datenow          = date("mY");
		$number_generator = $this->Stock_in_model->autonumber();
	    ...
	    if($id != 'new')
	    {
			...
			$header_data = $this->Stock_in_model->stockin_header($where)->row_array();
		...
		$this->twiggy_display('adm/Stock_in/edit', $data);
	}
	...
	{	
		...
		$data     = [];
		$get_data = $this->Stock_in_model->stockin_detail($where)->result();
	...
	public function get_embed()
	{
		...
		$this->twiggy_display('adm/stock_in/embed', $data);
	}
	...
	public function get_data_embed()
	{
		...
		$get_data  = $this->Stock_in_model->item_master($search)->result();
	...
	{	
		...
		{
			// save header
			$save_header = $this->Stock_in_model->save_header($header_data);
			if($save_header)
			{
				...
				$save_detail = $this->Stock_in_model->save_detail($detail_data, true);
			...
			$delete_detail = $this->Stock_in_model->delete_detail($id);

			if ($delete_detail) {
				$save_header = $this->Stock_in_model->update_header($id, $header_data);
	...
	public function delete()
	{
		...
		foreach($id as $row)
		{	
			$delete_header = $this->Stock_in_model->delete_header($row);
			
			if($delete_header){
				$delete_type = $this->Stock_in_model->delete_detail($row);
			}
		}
	...
	{	
		...
		$header = $this->Stock_in_model->purchaseorder_header($where_header)->row_array();
		$detail = $this->Stock_in_model->purchaseorder_detail($where_detail)->result();

		$data['content_title'] = 'Print Transaksi';
		$data['header'] = $header;
		$data['detail'] = $detail;

		$this->twiggy_display('adm/Stock_in/print_out', $data);
	}
