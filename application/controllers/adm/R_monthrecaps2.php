...
class R_monthrecaps2 extends BaseController
{
	...
	{	
		...
		foreach($daterange as $date){
			...
			$tunai = $fee_tunai_1;
			$trans = $fee_trans_1;
			$allmethod = $kartu + $tunai + $trans;
			$data[] = array(
				'no'       => $no,
				'guest'    => 1,
				'numberk'  => 1,
				'number'   => $date->format("d/m/Y"),
				'kartu'    => number_format(abs($kartu)),
				'tunai'    => number_format(abs($tunai)),
				'transfer' => number_format(abs($trans)),
				'total'    => number_format(abs($allmethod)),
				'desc'     => 1,
			);
