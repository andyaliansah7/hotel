...
class Checkin extends BaseController {
	...
	public function index() {
		...
		if(check_roles('1')){
			$this->twiggy_display('adm/checkin/index', $data);
		}else{
			redirect("Error");
		}
	}
	...
	public function save() {
		...
		$data_pricetype = $this->Room_types_model->get_data(array('room_type_id' => $room_type))->row_array();
		$weekday_prc    = $data_pricetype['room_type_price_weekday'];
		$weekend_prc    = $data_pricetype['room_type_price_weekend'];
		...
		if($id == 'new') {
			...
			$data_save_transaction = array(
				...
				'weekday_price'      => $weekday_prc,
				'weekend_price'      => $weekend_prc,
				'user_id'            => $user_id,
				'shift_id'           => $shift_id
			);
	...
	public function duplicate()
	{
		...
		for ($i = 0; $i < $row; $i++){
			...
			$data_save_transaction = array(
				...
				'date_in'            => $get_data->date_in,
				'date_out'           => $get_data->date_out,
				'interval_stay'      => 0,
				'room_price'         => 0,
				'total_price'        => 0,
				'discount'           => 0,
				...
				'deposit'            => 0,
				'user_id'            => $user_id,
				'weekday_price'      => $get_data->weekday_price,
				'weekend_price'      => $get_data->weekend_price,
	...
	public function checkout_process() {
		...
		$wt = 'id = "'.$id.'" and (total - deposit) <> total_paid';
		$gt = $this->Checkin_model->get_data($wt)->result();

		$wc = 'transaction_id = "'.$id.'" and cs_header_total <> cs_header_paid';
		$gc = $this->Consumption_services_model->cs_header($wc)->result();
	...
	public function check_room_available()
	{
		...
		$data = [];
		$room_available = $this->Rooms_model->get_available_room($room_type_id, $date_in, $date_out, $id, $b_id, $type='C')->result();
		...
		$special_where = array('a.room_type_id' => $room_type_id, 'special_rate_date' => $date_in);
		$special_price = $this->Special_rates_model->get_data($special_where)->row();

		$date_3 = date('Y-m-d', strtotime($date_out. '+0 day'));
	
		$daterange = new DatePeriod(new DateTime($date_in), new DateInterval('P1D'), new DateTime($date_out));
		...
		if($interval == 1){
			...
			foreach($daterange as $date){
				...
				if($date->format("D") == 'Sat'){
					$price_total += $normal_price->room_type_price_weekend;
				}else{
					$price_total += $normal_price->room_type_price_weekday;
				}
			}

			$price_per_night = $price_total/$interval;
		}
