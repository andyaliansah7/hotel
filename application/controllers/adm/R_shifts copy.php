...
class R_shifts extends BaseController
{
	...
	{	
		...
		{	
			...
			{	
				...
				if($get_data_check){
					...
					$rate = ($get_data_check->on_behalf == '' ? 0 : $get_data_check->room_price);
				}
		...
		// Summary
		$room_active            = count($this->Rooms_model->get_data(array('room_active' => '1'))->result());
		$room_nonactive         = count($this->Rooms_model->get_data(array('room_active' => '0'))->result());
		$room_sealeble          = $room_active;
		$room_occupaid          = $guest_group_total;
		$room_sold              = $guest_group_total;
		$complimentary          = count($this->Reports_model->payment_header('', '2')->result());
		$house_use              = count($this->Reports_model->payment_header('', '3')->result());
