	...
	{	
		...
		{	
			...
			{	
				...
				$getda_dep_kartu = $this->Reports_model->get_data_deposit('COALESCE(SUM(deposit_amount),0) as total_dep_kartu', $where_dep_kartu)->row();
				...
				$getda_dep_tunai = $this->Reports_model->get_data_deposit('COALESCE(SUM(deposit_amount),0) as total_dep_tunai', $where_dep_tunai)->row();
				...
				$getda_dep_trans = $this->Reports_model->get_data_deposit('COALESCE(SUM(deposit_amount),0) as total_dep_trans', $where_dep_trans)->row();
		...
		$where_deposit = array('a.deposit_date >=' => $date_1, 'a.deposit_date <=' => $date_2, 'deposit_amount !=' => '0', 'a.payment_method_id !=' => '0');
		if(isset($shift) && $shift != '')
		{
			$where_deposit = array('a.deposit_date >=' => $date_1, 'a.deposit_date <=' => $date_2, 'deposit_amount !=' => '0', 'a.payment_method_id !=' => '0', 'a.shift_id' => $shift);
		}
		$order_deposit = 'a.payment_date ASC';
		$get_deposit   = $this->Reports_model->get_data_deposit('', $where_deposit)->result();
		...
		{	
			...
			{	
				...
				if($get_row->payment_method_type == 'Kartu'){
					$kartu_dep = $get_row->deposit_amount;
				}

				if($get_row->payment_method_type == 'Tunai'){
					$tunai_dep = $get_row->deposit_amount;
				}

				if($get_row->payment_method_type == 'Transfer'){
					$transfer_dep = $get_row->deposit_amount;
				}
				...
				$data_deposit[] = array(
					...
					'amount'         => number_format($get_row->deposit_amount),
