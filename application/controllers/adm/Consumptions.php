...
class Consumptions extends BaseController {
	...
	public function index() {
		$data['content_title'] = 'Makanan & Minuman';

		if(check_roles('1')){
			$this->twiggy_display('adm/consumptions/index', $data);
		}else{
			redirect("Error");
		}
	}
	...
	public function get_data() {
		$data = [];
		$get_data = $this->Consumptions_model->get_data(array('cs_type' => 'C'))->result();
	...
	public function edit($id='new') {
		...
		if($id != 'new') {
			...
			$get_data = $this->Consumptions_model->get_data($where)->row_array();
		...
		$this->twiggy_display('adm/consumptions/edit', $data);
	}
	...
	public function save() {
		...
		if($id == 'new') {
			...
			$save = $this->Consumptions_model->save($data_save);
		} else {
			$convert = convert_button($action, $id);
			$save = $this->Consumptions_model->update($id, $data_save);
		}
	...
	public function delete() {
		...
		foreach($id as $row) {
			$delete_type = $this->Consumptions_model->delete($row);
		}
	...
	public function check_id() {
		...
		$check = $this->Consumptions_model->check_id($where);
