...
window.MASTER = (function ($) {
	...
	var renderCheckbox = function (data, type, full, meta) {
		var checkbox = '<input class="check-sub-master" type="checkbox" value="' + full['id'] + '">'
		if(full['id'] == '1' || full['id'] == '2' || full['id'] == '3'){
			checkbox = '';
		}
		return checkbox;
	}
	...
	var renderEdit = function (data, type, full, meta) {
		var url = APP.siteUrl + 'adm/payment_methods/edit/' + full['id'];
	...
	return {
		...
		urlDeleteData: window.APP.siteUrl + 'adm/payment_methods/delete',
		urlRequestData: window.APP.siteUrl + 'adm/payment_methods/get_data',
